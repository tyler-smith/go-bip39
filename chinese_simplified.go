...
func init() {
	...
	// e3721bbf
	checksum := crc32.ChecksumIEEE([]byte(chineseSimplifiedWordList))
	if fmt.Sprintf("%x", checksum) != "e3721bbf" {
		panic("chineseSimplifiedWordList checksum invalid")
	}
}

// ChineseSimplifiedWordList is a slice of mnemonic words taken from the bip39 specification
...
var chineseSimplifiedWordList = `的
一
是
在
不
