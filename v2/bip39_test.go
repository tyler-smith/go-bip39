...
const testVectorPassword = "TREZOR"

func TestGenerate(t *testing.T) {
	testForSize := func(t *testing.T, bs sizeParams) {
		for i := 0; i < 16; i++ {
			m, err := Generate(bs, English, "")
			assert.NoError(t, err)
			assert.EqualInt(t, bs.wordLen, len(m.Words()))
			assert.EqualInt(t, bs.entropyLen, len(m.Entropy()))
			assert.EqualInt(t, 64, len(m.Seed()))
			assert.EqualInt(t, bs.wordLen, len(parseRawMnemonic(m.String())))
		}
	}
	...
	_cryptoRandReader = errReader{}
	m, err := Generate(Size128Bits, English, "")
	assert.EqualError(t, errEAGAIN, err)
	assert.True(t, m == nil)
}

func TestFromEntropy(t *testing.T) {
	// Success cases
	for _, vector := range successVectors() {
		entropyBytes, err := hex.DecodeString(vector.entropy)
		assert.NoError(t, err)

		m, err := FromEntropy(English, entropyBytes, testVectorPassword)
		assert.NoError(t, err)
		assert.EqualString(t, vector.mnemonic, m.String())
		assert.EqualString(t, vector.seed, hex.EncodeToString(m.Seed()))
	}

	// Failure cases
	m, err := FromEntropy(English, []byte{}, "")
	assert.NotNil(t, err)
	assert.True(t, m == nil)
}
...
func TestFromString(t *testing.T) {
	// Success cases
	for _, vector := range successVectors() {
		m, err := FromString(English, vector.mnemonic, testVectorPassword)

		assert.NoError(t, err)
		assert.EqualString(t, vector.mnemonic, m.String())
		assert.EqualString(t, vector.seed, hex.EncodeToString(m.Seed()))
		assert.EqualString(t, vector.entropy, hex.EncodeToString(m.Entropy()))
	}

	// Failure cases
	for _, vector := range failureVectors() {
		m, err := FromString(English, vector.mnemonic, testVectorPassword)
		...
		switch err {
		case ErrChecksumIncorrect:
			fallthrough
		case ErrWordSetInvalidLength:
			continue
		}

		if err, ok := err.(ErrUnknownWord); ok {
			assert.True(t, strings.Contains(err.Error(), "unknown word"))
			continue
		}
...
func TestEntropyFromReader(t *testing.T) {
	for _, test := range []struct {
		...
		{rand.Reader, nil},
		{errReader{}, errEAGAIN},
		{iotest.HalfReader(rand.Reader), ErrEntropyNotFullyRead},
		{iotest.OneByteReader(rand.Reader), ErrEntropyNotFullyRead},
	} {
		_, err := entropyFromReader(Size128Bits.entropyLen, test.r)
...
func TestGetWordIndex(t *testing.T) {
	for _, lang := range []Language{
		...
		for expectedIdx, word := range lang.wordSet {
			actualIdx, ok := lang.GetWordIndex(word)
			assert.True(t, ok)
			assert.EqualInt(t, int(actualIdx), expectedIdx)
			assert.EqualString(t, word, lang.GetWord(actualIdx))
		}

		for _, word := range []string{"a", "set", "of", "invalid", "wordSet"} {
			actualIdx, ok := lang.GetWordIndex(word)
			assert.False(t, ok)
			assert.EqualInt(t, int(actualIdx), 0)
			assert.EqualString(t, lang.wordSet[0], lang.GetWord(actualIdx))
		}
	}
}

func TestPadByteSlice(t *testing.T) {
	assert.True(t, bytes.Equal([]byte{0}, padByteSlice([]byte{}, 1)))
	assert.True(t, bytes.Equal([]byte{0, 1}, padByteSlice([]byte{1}, 2)))
	assert.True(t, bytes.Equal([]byte{1, 1}, padByteSlice([]byte{1, 1}, 2)))
	assert.True(t, bytes.Equal([]byte{1, 1, 1}, padByteSlice([]byte{1, 1, 1}, 2)))
}

// vector is a test record for a single mnemonic.
type vector struct {
	entropy  string
	mnemonic string
	seed     string
...
// successVectors returns vectors that should generate valid Mnemonics without
// any errors.
func successVectors() []vector {
	return []vector{
		{
...
// failureVectors returns vectors that should cause errors.
func failureVectors() []vector {
	return []vector{
		{mnemonic: ""},
...
// errEAGAIN is a simulated EAGAIN error used for testing error handling.
var errEAGAIN = errors.New("eagain")

// errReader is an io.Reader for testing entropy generation that always returns
// and errEAGAIN.
type errReader struct{}

// Read always returns an EAGAIN error.
func (errReader) Read([]byte) (int, error) { return 0, errEAGAIN }
